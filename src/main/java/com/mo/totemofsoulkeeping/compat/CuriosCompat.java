...
public final class CuriosCompat {
     ...
     * @param registries 注册表访问器，用于序列化 ItemStack（1.21.1 数据组件化所需）
     */
    public static void recordCuriosSlots(Player player, CompoundTag parent, HolderLookup.Provider registries) {
        if (!isLoaded()) {
            return;
        }
        ...
        for (Map.Entry<String, ICurioStacksHandler> entry : handler.getCurios().entrySet()) {
            ...
            for (int i = 0; i < stacks.getSlots(); i++) {
                ...
                if (!stack.isEmpty()) {
                    slotTag.put(String.valueOf(i), stack.save(registries, new CompoundTag()));
                }
            }
            if (!slotTag.isEmpty()) {
     ...
     * <ul>
     *   <li>记录的物品与 {@code stack} 同物同组件（{@link ItemStack#isSameItemSameComponents}）</li>
     *   <li>该槽位当前在玩家身上存在</li>
     *   <li>该槽位当前为空</li>
     * </ul>
     ...
     * @param registries   注册表访问器，用于反序列化 ItemStack（1.21.1 数据组件化所需）
    ...
    public static boolean tryReturnToCurioSlot(Player player, ItemStack stack, CompoundTag curiosSlots,
        ...
        for (String identifier : curiosSlots.getAllKeys()) {
            ...
            for (String idxStr : slotTag.getAllKeys()) {
                ...
                ItemStack recorded = ItemStack.parseOptional(registries, slotTag.getCompound(idxStr));
                if (recorded.isEmpty() || !ItemStack.isSameItemSameComponents(stack, recorded)) {
                    continue;
                }
